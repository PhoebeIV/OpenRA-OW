...
then
	...
	then
		TITLE=$(zenity --title='Launch OpenRA' --list --hide-header --text 'Select game mod:' --column 'Game mod' 'Opposing Worlds' 'Red Alert' || echo "cancel")
		if [ "$TITLE" = "Opposing Worlds" ]; then MODARG='Game.Mod=ow'
		elif [ "$TITLE" = "Red Alert" ]; then MODARG='Game.Mod=ra'
		else exit 0
		fi
	else
		echo "Please provide the Game.Mod=\$MOD argument (possible \$MOD values: ra, ow)"
		exit 1
	fi
fi
