...
namespace OpenRA.Mods.AS.Traits
{
	...
	{
		[FieldLoader.Require]
		[Desc("Weapon used to shoot the projectile. Caution: make sure that this is an insta-hit weapon, otherwise will look very odd! Note: This must be the name of an already defined armament, ie. primary")]
	...
	public class PointDefense : ConditionalTrait<PointDefenseInfo>, IPointDefense, ITick
	{
		...
		bool IPointDefense.Destroy(WPos position, Player attacker, string type)
		{
			...
			if (!info.ValidStances.HasRelationship(self.Owner.RelationshipWith(attacker)))
				return false;
